...
const figure = {
    ...
    replace: (_, g1, g2) => {
        ...
        // console.log(g2)
        return `<figure class="que-figure"><img class="que-img" src="${window.location.origin}/assets/tutorial/img/${g2}"${width ? ` style="width: ${width}px;"` : ''
...
const fetchMarkdown = async () => {
    if (window.location.pathname.split('/')[1] === '' || window.location.pathname.split('/')[1] === 'index.html') {
        ...
        const res = await fetch(
            `${window.location.origin}/assets/tutorial/story/tutorial${PAGE_NAME}.md`
        );
        ...
        if (res.status == 200) {
            ...
            const markdown = '# 해당하는 튜토리얼이 없습니다.'
            return markdown;
        }
        ...
        // github page url로 배포
        const res = await fetch(
            `${window.location.origin}/${window.location.pathname.split('/')[1]}/assets/tutorial/story/tutorial${PAGE_NAME}.md`
        );
