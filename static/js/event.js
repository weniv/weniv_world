...
// 라이캣 이미지 프리로드
preloadImage([
    `${window.STATIC_URL}img/characters/licat-0.webp`,
    `${window.STATIC_URL}img/characters/licat-1.webp`,
    `${window.STATIC_URL}img/characters/licat-2.webp`,
    `${window.STATIC_URL}img/characters/licat-3.webp`,
    `${window.STATIC_URL}img/icon/icon-alert-circle.svg`,
]);
...
const createCertifImg = (chapter) => {
    const img = new Image();
    img.src = `${window.STATIC_URL}img/certif-${chapter}.jpg`;
    img.onload = () => {
