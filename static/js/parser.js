...
const parser = (markdown) => {
    ...
    const figure = {
        ...
        replace: (_, g1, g2) => {
            ...
            return `<figure class="md-figure"><img class="md-img" loading="lazy" src="${window.STATIC_URL}data/story/img/${g2}" ${
...
const fetchMarkdown = async () => {
    // Django API에서 스토리 데이터 가져오기
    const response = await fetch('/api/stories/');
    const apiData = await response.json();
    const mdArr = [];

    if (response.status == 200 && apiData.success) {
        for (const story of apiData.stories) {
            if (story.content) {
                const parsedMarkdown = parser(story.content);
                const infoObj = {
                    id: story.id,
                    title: story.title,
